...
**事实基准：can-voice 的 `fix/client-defects` @ `6a12cb3`。那条分支仍在动**——
写这一段的过程中它又落了一个提交（`#46 电台行`），所以下面每一条都以该提交为准，
而不是以"今天的工作树"为准。
...
其中好几条确实没带过来。截至 `6a12cb3`，21 条修在还开着的 PR #51 上，**10 条仍未做**：
#29（i18n）、#34、#36（观察员模式）、#38、#40、#42、#44、#45、#47、#50（签名与版本号）。
...
| 7 msfs | 已落地 | 注入他机的姿态**少取一次负号**（新发现，无 issue） |
| 8 服务端通播机器人 | 已完成 | —— |
...
报文，"先起客户端写稿子"这件事本身就不成立；#47 的六条运行期退化（上线后不能切
跑道构型、不能手动推进字母、刷新周期写死 300 秒、登录 rating 写死观察员、模板拼错
不再提示、TTS 每轮重合成没有缓存）。
...
- [ ] **新发现，不在任何 issue 里：注入他机的姿态少取了一次负号**
...
- [ ] **版本号不注入** —— #50 的前一半，而且它反过来咬 Task 3 Step 5
...
| R4 | **签名。仍然是未决，而且已经在发无签名的包了** | Windows 包已经发出去三版，release 正文里直接把 SmartScreen 的蓝色警告写给用户；macOS 因为同一张证书**根本没有构建**，那个平台上装不上任何一个客户端。见 Task 9 |
| R5 | 44,800 行 Python 的领域知识 | **9-16 的审计就是"归档前再过一遍 `CLAUDE.md`"这件事，开出 31 条**，其中 10 条仍未做。R5 从"应当做"变成了"做过一轮，结果在 issue 里"——但它只覆盖了四个客户端，`can-audio` 的服务端那一半还没过 |
...
| R8 | **新发现：注入进 MSFS 的他机姿态少取一次负号** | 读路径取负、写路径没取（`msfs.rs:478-479,867-868,906-907`），注入的他机俯仰坡度应当是反的。**单机自测看不出来，要两个人对飞才看得见**，而且没有任何测试覆盖注入侧的符号。见 Task 7 |
